// Forked from Game/Board/ProceduralTrackMesh.cs for the XR edition (XR-PRD X20): a copy, never kept in sync.
...
namespace TrainSudoku.XR
{
    ...
    /// are split into short boxes so the bender can curve them. Replaced by the kit mesh through <see cref="XRBoardAssets"/>.
    /// </summary>
        ...
        private sealed class BoxMeshBuilder
        {
            ...
            public void AddBox(Vector3 center, Vector3 size)
            {
                ...
                foreach (var normal in FaceNormals)
                {
                    ...
                    // in practice only because XRBoardAssets ships with the kit mesh assigned.
