...
- The main camera carries a `BoardCamera` (added at runtime) that solves `CameraFit` for the board extents and re-fits when the aspect changes. The Play HUD is transparent apart from a small `||` pause button, the level name and the clock along the top edge. The pause menu shows an editor-only "Debug: win" button that resumes and completes the level until M7 wires the validator.
