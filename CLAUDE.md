...
- Progress lives in `save.json` under `Application.persistentDataPath` (`GameManager.SaveFilePath`), written through on every new best by `FileSaveStore` with a temp-file swap; a corrupt file is set aside as `.corrupt`. The format is `{"version":1,"bestTimes":{"level-id":seconds}}`, read and written by the hand-rolled `SaveJson` so Core needs no UnityEngine. The Game inspector has "Delete save file" for testing.
